...
void *producer(void *parameters){
  ...
  for(;;){
    ...
    if(!env_is_set && !env_found){
      env_found = predict_enviroment(be64toh(received_task->start), be64toh(received_task->end) - be64toh(received_task->start));
      if (env_found){
    ...
    if(env_found){
      prediction = get_prediction(be64toh(received_task->end) - be64toh(received_task->start));
    } 
    if (prediction && prediction_holds(prediction, received_task->hash)){
      prediction = htobe64(prediction);
      if (send(received_task->client, &prediction, PACKET_RESPONSE_SIZE, 0) != PACKET_RESPONSE_SIZE){
        printf("Can't send\n");
        exit(1);
      }
      // Closing the socket:
      close(received_task->client);
      free(received_task);
    } else {
      // enqueue the task
